...
def get_input_data() -> pd.DataFrame:
    ...
    df1.columns = df1.columns.str.strip()  # ✅ Add this
    df1.rename(columns={'Type 1': 'y1', 'Type 2': 'y2', 'Type 3': 'y3', 'Type 4': 'y4'}, inplace=True)

    df2 = pd.read_csv("/Users/ahadshaikh/Desktop/CA_EEAS/email-multilabel-classifier/data/Purchasing.csv", skipinitialspace=True)
    df2.columns = df2.columns.str.strip()  # ✅ Add this
    df2.rename(columns={'Type 1': 'y1', 'Type 2': 'y2', 'Type 3': 'y3', 'Type 4': 'y4'}, inplace=True)

    df = pd.concat([df1, df2])
    df.columns = df.columns.str.strip()  # ✅ Final cleanup for merged DataFrame
...
def preprocess_data(df: pd.DataFrame, text_column: str = 'Interaction content'):
    ...
    y2 = le2.fit_transform(df['y2'].astype(str))
